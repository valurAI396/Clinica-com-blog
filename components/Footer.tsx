    ...
    <footer className="bg-white pt-20 pb-10 rounded-t-[3rem] -mt-10 relative z-0">
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 lg:gap-16">
          ...
          {/* Quick Links */}
          <div>
            <h4 className="font-semibold text-stone-800 mb-6">{t('footer.quickLinks')}</h4>
            <ul className="space-y-3">
              <li><Link to="/" className="text-stone-500 hover:text-clarity-primary text-sm transition-colors">{t('nav.home')}</Link></li>
              <li><Link to="/servicos" className="text-stone-500 hover:text-clarity-primary text-sm transition-colors">{t('nav.services')}</Link></li>
              <li><Link to="/equipa" className="text-stone-500 hover:text-clarity-primary text-sm transition-colors">{t('nav.team')}</Link></li>
              <li><Link to="/blog" className="text-stone-500 hover:text-clarity-primary text-sm transition-colors">{t('nav.blog')}</Link></li>
              <li><Link to="/contactos" className="text-stone-500 hover:text-clarity-primary text-sm transition-colors">{t('nav.contact')}</Link></li>
            </ul>
          </div>

          {/* Contact */}
          <div className="md:col-span-2">
            <h4 className="font-semibold text-stone-800 mb-6">{t('footer.contact')}</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
              <ul className="space-y-4">
                <li className="flex items-start gap-3 text-stone-500 text-sm">
                  ...
                  <span className="whitespace-pre-line">{currentContactInfo.address}</span>
                </li>
              </ul>
              <ul className="space-y-4">
                <li className="flex items-start gap-3 text-stone-500 text-sm">
                  ...
                  <div className="flex flex-col gap-1">
                    {currentContactInfo.phone.split(' / ').map((line, idx) => (
                ...
                <li className="flex items-center gap-3 text-stone-500 text-sm">
                  ...
                  <span>{currentContactInfo.email}</span>
                </li>
        ...
        <div className="mt-16 pt-8 border-t border-stone-100 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-stone-400 text-xs">
            &copy; {new Date().getFullYear()} Psicologia do 1º Andar. {t('footer.rights')}
          </p>
